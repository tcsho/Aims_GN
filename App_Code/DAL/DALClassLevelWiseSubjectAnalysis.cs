...
/// <summary>
/// Loads rows for Class Level Wise Subject Analysis from LmsAppReports.Rpt_View (same filter model as EYE classwise report).
/// </summary>
public class DALClassLevelWiseSubjectAnalysis
{
    ...
    public DataTable GetReportData(BLLClassLevelWiseSubjectAnalysis p)
    {
        ...
        using (var cmd = new SqlCommand(sql, cn))
        {
            ...
            cmd.CommandTimeout = ResolveCommandTimeoutSeconds();
    ...
    private static void AppendSessionFilter(StringBuilder where, List<SqlParameter> prms, BLLClassLevelWiseSubjectAnalysis p)
    {
    ...
    {
        string s = ConfigurationManager.AppSettings["ClassLevelWiseSubjectAnalysisSqlCommandSeconds"];
        if (string.IsNullOrWhiteSpace(s))
            s = ConfigurationManager.AppSettings["EYEClassWiseAnalysisSqlCommandSeconds"];
        int sec;
