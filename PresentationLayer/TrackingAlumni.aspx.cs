...
public partial class PresentationLayer_TrackingAlumni : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                else if (UserLevel_ID == 4 && Session["UserType_Id"].ToString() == "34")//Counselors   //Addded by Hasan
                {
                    ...
                    list_counselors.Enabled = false;    //2023-19-oct
                }
                ...
                else if (UserLevel_ID == 5) //Teacher
                {
                    ...
                    list_counselors.SelectedValue = row["User_Id"].ToString();
                    list_counselors.Enabled = false;
                ...
                if (list_counselors.Enabled == false)
                {
                    lab_teacher.Text = list_counselors.SelectedItem.Text;
                    list_counselors.Visible = false;
                }
    ...
    private void BindGrid()
    {
        try
        {
            ...
            //objSer.Section_Id = list_section.SelectedValue;
            objSer.Main_Organisation_Id = Session["moId"].ToString();
            objSer.Us_AddTag = list_counselors.SelectedValue;
            //objSer.Teacher_Id = list_teacher.SelectedValue;
            objSer.EndIndex = Session["isClassTeacher"].ToString();
            objSer.StartIndex = "";


            dt = objSer.TrackAlumni(objSer);
        ...
        try
        {
            if (list_center.SelectedValue == "")
            {
               // list_teacher.Items.Clear();
                ...
                list_counselors.Items.Clear();
                //****list_counselors.Items.Insert(0, new ListItem("Select", ""));
            }
            else
            {
                ...
                BLLUploadEducationalArchive objcoun = new BLLUploadEducationalArchive();

                int centerid = Int32.Parse(list_center.SelectedValue.ToString());
                DataTable dtcoun = objcoun.CounselorsSelectByCenterIDfor_Alumni(centerid);
                objBase.FillDropDown(dtcoun, list_counselors, "USER_ID", "First_Name");
            }
        ...
        if (FileUpload1.HasFile)
        {
            ...
            //foreach (HttpPostedFile postedFile in FileUpload1.PostedFiles)
            {
                ...
                if (1 == 1)
                {
                    ...
                    if (Extension.ToLower() == ".doc" || Extension.ToLower() == ".jpg" || Extension.ToLower() == ".jpeg" || Extension.ToLower() == ".docx" || Extension.ToLower() == ".pdf" || Extension.ToLower() == ".png" || Extension.ToLower() == ".xlsx" || Extension.ToLower() == ".xls")
                    {
                        string MaxId;
