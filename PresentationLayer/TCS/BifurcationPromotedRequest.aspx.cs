...
public partial class PresentationLayer_BifurcationPromotedRequest : System.Web.UI.Page
{
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            //Added By Huzaifa for Class 9M//
            AddOrRemoveClass9M(gvr.Cells[1].Text, gv_details.DataKeys[gvr.RowIndex].Values["PromotedTo"].ToString());
    ...
    protected void btnSubmitReq_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            objClsSec.Remarks = txtRemarks.Text;
            objClsSec.ClassRequest = Convert.ToInt32(rdbPromotionClass.SelectedValue);
    ...
    protected void gv_details_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        {
            Label hfStatus = (e.Row.FindControl("lblolevel") as Label);
 	    Label EmailStatus = (e.Row.FindControl("emailstatus") as Label);
            Button btnSendUndertaking = (Button)e.Row.FindControl("btn_bif_email");
            //  e.Row.Cells[9].Visible = false;
            //DataRow row = ((DataRowView)e.Row.DataItem).Row;
            if (ddlClass.SelectedValue == "12" && ddlterm.SelectedValue == "1" && (ddl_region.SelectedValue == "30000000" || ddl_region.SelectedValue == "40000000"))
            {
                hfStatus.Visible = true;
            }
            else if (ddlClass.SelectedValue == "12" && ddlterm.SelectedValue == "2" && ddl_region.SelectedValue == "20000000")
            {
                hfStatus.Visible = true;
            }
	    else if (ddlClass.SelectedValue == "13" || ddlClass.SelectedValue == "14")
            {
                EmailStatus.Visible = false;
                btnSendUndertaking.Visible = false;
            }
            else
            {

                hfStatus.Visible = false;
            }

            //foreach (GridViewRow itemrow in gv_details.Rows)
            //{
            //    //EmailList.Add(itemrow.Cells[YourIndex].Text);
            //    itemrow.Cells[9].Visible = false;
            //}
        }
    ...
    DataTable SP_REMINDER_EMAIL_IEP_BIFURCATION(string sSessionID, string sCenterID, string SStudentID = "", string sStudentName = "", string sClassID = "", int term = 0)
    {
        DataTable DT_Data = null;
        obj_Access.CreateProcedureCommand("SP_REMINDER_EMAIL_IEP_BIFURCATION");
        obj_Access.AddParameter("P_SessionID", sSessionID, DataAccess.SQLParameterType.VarChar, true);//
        obj_Access.AddParameter("P_CenterID", sCenterID, DataAccess.SQLParameterType.VarChar, true);
        obj_Access.AddParameter("P_StudentID", SStudentID, DataAccess.SQLParameterType.VarChar, true);
        obj_Access.AddParameter("P_StudentName", sStudentName, DataAccess.SQLParameterType.VarChar, true);
    ...
    protected void btn_view_Undertkng_Click(object sender, EventArgs e)
    {
        ...
        ViewState["Section_Name"] = gvr.Cells[5].Text;
        ViewState["TermGroupID"] = gv_details.DataKeys[gvr.RowIndex].Values["TermGroupID"].ToString();
        Response.Redirect("~/PresentationLayer/IEP_Undertaking_Bifurcation.aspx?S=" + txtStdId.Text + "&C=" + "" + "&T=" + ddlterm.SelectedValue);
    }
        ...
        if (dt2.Rows.Count > 0)
        {
            if (dt2.Rows[0]["Completion"].ToString() == dt2.Rows[0]["Total"].ToString())
            {
                ...
                if (dt1.Rows.Count > 0)
                {
                    ...
                    //var Email = new MailAddress("AppNotifications@csn.edu.pk", "The City School");
                     var Email = new MailAddress("noreply@csn.edu.pk", "The City School");
                    To = "moiz.galmani@csn.edu.pk";//dt1.Rows[0][2].ToString();
                    CC = "parvaiz.hussain@csn.edu.pk";//dt1.Rows[0]["centerEmail"].ToString() + ';' + "ManageAcknowledgement@csn.edu.pk";
                                                      // CC = "ManageAcknowledgement@csn.edu.pk";  //
                    ...
                    Body += "<p style='margin:0 0 12px 0;font-size:16px;line-height:24px;font-family:Arial,sans-serif;'>If you wish for your child to continue on the O-Level route, then please read & submit the application for undertaking.</p><br/>";
                    Body += "<p style='margin: 0 0 12px 0font - size:14pxline - height:24pxfont - family:Arial,sans - serif'>I, as a parent / guardian of <strong> " + dt1.Rows[0]["First_Name"].ToString() + "</strong>ERP # " + dt1.Rows[0]["Student_No"].ToString() + " </strong> studying in " + dt1.Rows[0]["Class_Name"].ToString() + "</strong>  " +
                        "confirm that I have fully read and understood the points below. My acknowledgement indicates full agreement and consent to apply the appropriate consequences stated:</p> ";
                    ...
                    var urlconfirmation = "http://trainingaims.thecityschool.edu.pk/PresentationLayer/Bifurcation_Confirmation.aspx?St_Id=" + encrypted_student_Id + "&Cl_Id=" + encrypted_class_Id + "&Sess_Id=" + encrypted_session_Id + "&St_Nm=" + encrypted_student_Name + "&St_ClS=" + encrypted_student_ClassSec;//ddlStudent.SelectedValue
                    // var url = "http://localhost:50091/PresentationLayer/Bifurcation_Confirmation.aspx?St_Id=" + encrypted_student_Id+"&Cl_Id="+ encrypted_class_Id+"&Sess_Id="+ encrypted_session_Id+"&St_Nm=" + encrypted_student_Name + "&St_ClS=" + encrypted_student_ClassSec;//ddlStudent.SelectedValue
                   // var urlconfirmation = "http://tcsaims.com/PresentationLayer/Bifurcation_Confirmation.aspx?St_Id=" + encrypted_student_Id + "&Cl_Id=" + encrypted_class_Id + "&Sess_Id=" + encrypted_session_Id + "&St_Nm=" + encrypted_student_Name + "&St_ClS=" + encrypted_student_ClassSec;//ddlStudent.SelectedValue
                    var urliep = "http://www.tcsaims.com/PresentationLayer/tcs/Parent_IEP_Form.aspx?s=" + txtStdId.Text + "&ses=" + Session["Session_Id"].ToString();
                    //  Body += "<p style='text-align:center'><b><a style='color: #fff text-decoration:none border:none padding:10px 100px !important background:#0C4DA2 border-radius:10px 'href='" + urliep + "' >Click to view IEP form</a></b></p> ";
                    Body += "<p style='text-align:center'><b><a style='color: #fff;text-decoration:none;border:none;padding:10px 100px !important;background:#0C4DA2;border-radius:10px;' href='" + urlconfirmation + "'>Confirm</a></b></p>";
                    ...
                    try
                    {
                        using (MailMessage mm = new MailMessage(Email.Address, To))
                        {
                            ...
                            mm.Subject = "Acknowledgement Reminder to Parents for Undertaking";
                            mm.From = new MailAddress("AppNotifications@csn.edu.pk", "The City School");
                            mm.CC.Add(new MailAddress(CC));
                            //mm.From = new MailAddress("AppNotifications@csn.edu.pk", "The City School");
