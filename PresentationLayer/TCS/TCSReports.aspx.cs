...
public partial class PresentationLayer_TCS_TCSReports : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (id == 341)
                {
                    rblReportType_SelectedIndexChanged(sender, e);
                }
    ...
    protected void FillClassExcludingEYE()
    {
        try
        {
            DataTable dt = new BLLClass().ClassFetch(new BLLClass());

            int[] excludedClassIds = { 2, 3, 4, 5, 6 };

            // Filter DataTable based on excludedClassIds
            DataTable filteredDataTable = dt.AsEnumerable()
                .Where(row => !excludedClassIds.Contains(row.Field<int>("Class_Id")))
                .Any()
                ? dt.AsEnumerable()
                    .Where(row => !excludedClassIds.Contains(row.Field<int>("Class_Id")))
                    .CopyToDataTable()
                : dt.Clone();
            // Populate DropDownList and CheckBoxList
            objBase.FillDropDown(filteredDataTable, ddlClass, "Class_Id", "Name");
            BindCheckBoxListControl(filteredDataTable, lstClass, "Class_Id", "Name");
        }
        catch (Exception ex)
        {
            Session["error"] = ex.Message;
    ...
    protected void FillClassAlumni()
    {
        try
        {
            BLLClass objBLLClass = new BLLClass();
            DataTable dt = null;             
            dt = objBLLClass.ClassSelectForAlumni(objBLLClass); 
