...
public partial class PresentationLayer_TCS_IEP_TeacherCounsellorReport : System.Web.UI.Page
{
    ...
    public DataTable exec_SP(string Center, string Class, string Action)
    {
        ...
        DataTable dt = objBase.sqlcmdFetch("sp_IEPReport_TeacherCounselorInCompleted_IEP", param);
        dt.Dispose();
        
        return dt;
        //Sp_IEP_TeacherCounsellorReport_Table
    }
    ...
    protected void Grid_IEPStudents_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        {
            int teacherColumnIndex = 7;
            string teacherValue = e.Row.Cells[teacherColumnIndex].Text;
            if (teacherValue.Contains(" out of 0"))
            {
                e.Row.Cells[teacherColumnIndex].Text = "0 out of 0";
            }
            else
            {
                e.Row.Cells[teacherColumnIndex].Text = teacherValue;
            }
            int ReasonColumnIndex = 8;
            string ReasonValue = e.Row.Cells[ReasonColumnIndex].Text;
            if (ReasonValue.Contains(" out of 0"))
            {
                e.Row.Cells[ReasonColumnIndex].Text = "0 out of 0";
            }
            else
            {
                e.Row.Cells[ReasonColumnIndex].Text = ReasonValue;
            }

            int Reason1ColumnIndex = 8;
            string Reason1Value = e.Row.Cells[Reason1ColumnIndex].Text;
            if (Reason1Value.Equals("0 out of 0"))
            {
                e.Row.Cells[Reason1ColumnIndex].Text = "Joined after MYE";
            }
            else
            {
                e.Row.Cells[Reason1ColumnIndex].Text = string.Empty;
            }
    ...
    public DataTable exec_SP_table(string Center, string Class, string Action)
    {
        ...
        DataTable dt = objBase.sqlcmdFetch("Sp_IEP_TeacherCounsellorReport_Table", param);
        dt.Dispose();
        return dt;
