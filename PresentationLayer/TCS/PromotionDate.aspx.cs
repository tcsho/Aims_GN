...
public partial class PresentationLayer_TCS_PromotionDate : System.Web.UI.Page
{
    ...
    private void BindIssuanceDateGrid()
    {
        try
        {
            ...
            var dt = bllIssuanceDate.Student_Promotion_Date_All(bllIssuanceDate.Session_Id);
    ...
    protected void FillSessions()
    {
        try
        {
            ...
            //foreach (DataRow r in dt.Rows[Convert.ToInt32(ddlSession.Items.Count - 2)])
            //{

                ddlSession.SelectedValue = dt.Rows[Convert.ToInt32(ddlSession.Items.Count - 2)]["Session_Id"].ToString();
                ddl_Session.SelectedValue = dt.Rows[Convert.ToInt32(ddl_Session.Items.Count - 2)]["Session_Id"].ToString();
            //ddlSession.SelectedIndex = ddlSession.Items.Count - 2;
            //ddl_Session.SelectedIndex = ddl_Session.Items.Count - 2;
            //}
   ...
   public void AddIssuanceDate(object sender, EventArgs e)
    {
        try
        {
            ...
            if (button.Text == "Add")
            {
                bllIssuanceDate.Session_Id = Convert.ToInt32(ddl_Session.Text);
                bllIssuanceDate.DateFrom = Convert.ToDateTime(txtFrmDate.Text);               
 

                var k = bllIssuanceDate.AddPromotionDate(bllIssuanceDate);
            ...
            else
            {
                ...
                var k = bllIssuanceDate.UpdatePromotionDate(bllIssuanceDate);
                if (k == 0)
                {
                    BindIssuanceDateGrid();
                    ResultIssue.Visible = true;
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        lblModalTitle.Text = "Add Promotion Date";
        if (btn.Text == "Update")
        {
            ...
            {
                ddl_Session.SelectedValue = words[2];
            }
            else
            {
