...
using System.Web.UI;
public partial class PresentationLayer_SearchStudentAlevel : System.Web.UI.Page
{
    private int windowWidth = 5;
    DALBase objBase = new DALBase();
    int UL_ID;
    BLLSearchStudentAlevel objSer = new BLLSearchStudentAlevel();
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                list_studentStatus.Enabled = false;
                ...
                list_class.SelectedValue = "19";
                list_class_SelectedIndexChanged(null, null);
                #endregion
    ...
    private void BindGrid()
    {
        try
        {
            ...
            objSer.Subject_Id = list_subject.SelectedValue;
            ...
            objSer.Teacher_Id = list_teacher.SelectedValue;
            objSer.Subject_Id = list_subject.SelectedValue;
            objSer.Status = list_status.SelectedValue;
    ...
    protected void list_class_list_teacher_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            if (list_class.SelectedValue != "" && list_teacher.SelectedValue != "")
            {
    ...
    private void BindSubject()
    {
        try
        {
            ...
            obj.Class_Id = Convert.ToInt32(list_class.SelectedValue.ToString());
            obj.Employee_Id = Convert.ToInt32(list_teacher.SelectedValue.ToString());

            DataTable dt = (DataTable)obj.Techer_SubjectSelectByClassId(obj);
    ...
    {
        string Class_Id = list_class.SelectedValue;
        string Teacher_Id = list_teacher.SelectedValue;
        string Subject_Id = list_subject.SelectedValue;
        string Center_Id = list_center.SelectedValue;

        string Status = list_status.SelectedValue;
        ...
        foreach (GridViewRow gvr in dg_student.Rows)
        {
            ...
            if (chkAllowAccess.Checked)
            {
                if (Status == "0")
                {
                    ...
                    {
                        int AlreadyIn = 0;
                        if (Class_Id != "" && Teacher_Id != "" && Subject_Id != "" && Center_Id != "")
                        {
                            objClsSec.Center_Id = Convert.ToInt32(Center_Id);
                            objClsSec.Session_Id = Convert.ToInt32(Teacher_Id);
                            objClsSec.User_Id = Convert.ToInt32(Session["ContactID"].ToString());
                            objClsSec.Class_Id = Convert.ToInt32(Class_Id);
                            objClsSec.Section_Id = Convert.ToInt32(Subject_Id);
                            objClsSec.Student_Id = Convert.ToInt32(gvr.Cells[2].Text);

                            AlreadyIn = objClsSec.TCS_Assign_Student_By_Subject_Id(objClsSec);
                            ImpromptuHelper.ShowPrompt("Student Assign Successfully!");
                ...
                {
                    ImpromptuHelper.ShowPrompt("Student already assigned");
                }
            }
            else
            {
                if (Status == "1")
                {
                    try
                    {
                        ...
                        if (Class_Id != "" && Teacher_Id != "" && Subject_Id != "" && Center_Id != "")
                        {
                            ...
                            objClsSec.Section_Id = Convert.ToInt32(Subject_Id);
                            objClsSec.Student_Id = Convert.ToInt32(gvr.Cells[2].Text);
                            objClsSec.TermGroup_Id = Convert.ToInt32(Status);

                            AlreadyIn = objClsSec.TCS_Assign_Student_By_Update_Subject_Id(objClsSec);
                            ImpromptuHelper.ShowPrompt("Student Un-Assign Successfully!");
