...
public partial class PresentationLayer_SearchStudent : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (!Page.IsPostBack)
            {
               ...
               // scriptManager.RegisterPostBackControl(this.but_Export);
                ...
                //this.Page.Title = "SearchStudent - Copy";
    ...
    private List<string> GetUserList()
    {
        ...
        foreach (GridViewRow row in dg_student.Rows)
        {
            ...
            if (checkbox != null && checkbox.Checked)
            {
                ...
                // Assuming Student_Id is in the second cell
                if (list_UserType.SelectedValue == "Student")
                {
                    studentIdCell = row.Cells[1];
                }
                else
                {
                    studentIdCell = row.Cells[2];
                }

                string studentId = studentIdCell.Text;

                // Add the checked Student_Id to the list
    ...
    private void SendPushNotification(List<string> userList)
    {
        ...
        var requestObject = new
        {
            notification = new
            {
                title = pushMessageTitle.Text,
                body = pushMessageBody.Text
            },
            userName = userList.ToArray()
        };
        ...
        var request = new HttpRequestMessage(HttpMethod.Post, "http://app.csn.edu.pk:8080/api/SendPushNotification");
        ...
        {
            list_region.SelectedValue = "0";
            list_class.SelectedValue = "0";
            list_section.SelectedValue = "0";
            list_UserType.SelectedValue = "0";
            dg_student.DataSource = null;
            PushMessage.Visible = false;
            dg_student.DataBind();
            ImpromptuHelper.ShowPrompt("Notification Sent Successfully!");
        }
    ...
    {
        CheckBox chkSelectAll = (CheckBox)dg_student.HeaderRow.FindControl("chkSelectAll");
    ...
    protected void list_userTypes_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
    ...
    private void BindGrid()
    {
        try
        {
            if (list_UserType.SelectedValue != "0")
            {
                ...
                objSer.Region_Id = list_region.SelectedValue;
                objSer.Center_Id = list_center.SelectedValue;
                objSer.Class_Id = Convert.ToInt32(list_class.SelectedValue);
                objSer.Section_Id = list_section.SelectedValue;
                objSer.Main_Organisation_Id = Session["moId"].ToString();
                ...
                dt = GetDataOfRegisteredStudentParent(objSer);
                ...
                if (list_UserType.SelectedValue == "Parent")
                {
                    dg_student.Columns[1].Visible = false;
                    dg_student.Columns[2].Visible = true;
                    dg_student.Columns[3].Visible = false;
                }
                else if (list_UserType.SelectedValue == "Student")
                {
                    dg_student.Columns[1].Visible = true;
                    dg_student.Columns[2].Visible = false;
                    dg_student.Columns[3].Visible = true;
                }
                dg_student.DataBind();
            ...
            else
            {
                dg_student.DataSource = null;
                PushMessage.Visible = false;
                // Rebind the GridView to refresh it
                dg_student.DataBind();
            }
        }
        catch (Exception ex)
    ...
    protected void list_class_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            DataRow row = (DataRow)Session["rightsRow"];
            if (list_center.SelectedValue != "0")
            {
                ...
                else
                {
                    ...
                    objSec.Center_Id = Convert.ToInt32(list_center.SelectedValue.ToString());
    ...
    string connectionString = Convert.ToString(System.Configuration.ConfigurationManager.ConnectionStrings["isl_amsoConnectionString"]);
    public DataTable GetDataOfRegisteredStudentParent(BLLSearchStudent obj)
    {
        ...
        try
        {
            ...
            _dt = dalobj.sqlcmdFetch("selectStudentForNotification", param);
            return _dt;
        }
