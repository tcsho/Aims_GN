...
public partial class PresentationLayer_New_Bifurcation_Confirmation : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        string decrypted_Session_Id = System.Text.ASCIIEncoding.ASCII.GetString(sessd);
         ...
         DataTable dt1 = ExecuteProcedure_NEW_StudentDetail(decrypted_Student_Id, "");
         string studentname = dt1.Rows[0][1].ToString();
         string centerid = dt1.Rows[0][12].ToString();
        string Term = dt1.Rows[0][8].ToString();
        string region_id = dt1.Rows[0][11].ToString();
         dt1.Dispose();
        // /****EMAIL***/
        ...
        var CC2 = "ManageAcknowledgement@csn.edu.pk";
        var Subject = "Parent's Undertaking for " + studentname + " (" + decrypted_Student_Id + ") | " + dt1.Rows[0][4].ToString() + " | Term: " + Term + " | " + dt1.Rows[0]["Center_Name"].ToString();
        ...
        DataTable dtupdate = ExecuteProcedure(decrypted_Session_Id, "", decrypted_Student_Id, "", decrypted_Class_Id, Term);
        dtupdate.Dispose();
    ...
    DataTable ExecuteProcedure(string sSessionID, string sCenterID, string SStudentID = "", string sStudentName = "", string sClassID = "",string p_term = "")
    {
        DataTable DT_Data = null;
        obj_Access.CreateProcedureCommand("NEW_IEP_AutoMail_IEP_UT_Status_Update");
