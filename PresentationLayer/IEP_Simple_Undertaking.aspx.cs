...
public partial class PresentationLayer_TCS_IEP_Simple_Undertaking : System.Web.UI.Page
{
        ...
        try
        {
            if (!Page.IsPostBack)
            {
                ...
                if (Request.QueryString.Count > 0)
                {
                    ...
                    DataTable dt = ExecuteProcedure_NEW_StudentDetail(_studntid, _secid, Convert.ToInt32(_Trmid));
                    dt.Dispose();
                    string _classID = "";
                    if (dt.Rows.Count > 0)
                    {
    ...
    /**detail procedure**/
    DataTable ExecuteProcedure_NEW_StudentDetail(string student_id, string section_id, int term_id)
    {
        DataTable DT_Data = null;
        obj_Access.CreateProcedureCommand("sp_NEW_IEP_studentdetail");
